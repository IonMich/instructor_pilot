...
def get_serialized_submissions(student_pk, course_pk, assignment_group=None):
    ...
    submissions = PaperSubmission.objects.filter(Q(assignment__course=course))
    if assignment_group:
        submissions = submissions.filter(
            assignment__assignment_group_object=assignment_group
        )
    ...
    student_submissions = [
        {
            ...
            "front_page": submission.submissions_papersubmissionimage_related.filter(
                page=1
            )
            .first()
            .image.url,
...
def detail_view(request, course_pk, student_pk):
    ...
    student_submissions, remaining_submissions = get_serialized_submissions(
