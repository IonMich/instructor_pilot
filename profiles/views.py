...
def profile_preferences_edit_view(request):
    ...
    # user preferences JSON field in the database. 
    # Currently the accepted preferences are:
    ...
    for preference in data:
        preference_data = data.get(preference, None)
        user_preference = preferences.get(preference, None)
        if not preference_data:
            return JsonResponse({"error": "No data for submitted preference {}".format(preference)}, status=400)
        ...
        preferences[preference] = user_preference
        profile.preferences = preferences
        profile.save()
    return JsonResponse({"success": True})

@login_required
def api_get_profile_preferences(request):
    if request.method != "GET":
        return HttpResponseNotFound()
    # we return the preferences of the user
    user = request.user
    profile = user.profile
    preferences = profile.preferences
