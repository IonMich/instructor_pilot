...
import { sectionsQueryOptions, studentsOfSectionQueryOptions } from '@/utils/queryOptions';
import { seo } from '@/utils/utils'
import { createFileRoute, useLoaderData } from '@tanstack/react-router'

function getBreadcrumbItems(course: Course, section: Section, student: Student) {
    return [
        {
            title: "Home",
            path: "/",
        ...
        {
            title: (student.first_name && student.last_name) ? `${student.first_name} ${student.last_name}` : `Student ${student.id}`,
            path: `/courses/${course.id}/students/${student.id}`,
        },
    ];
  }

export const Route = createFileRoute('/_authenticated/courses/$courseId/students/$studentId') ({
    parseParams: (params) => ({
                ...
                if (student.id === studentId) {
                    for (const sectionOfStudent of student.sections) {
                        ...
                        return {
                            ...
                            breadcrumbItems: getBreadcrumbItems(section.course, section, student),
                        }
                    }
                }
            }
...
function StudentDetail() {
    const data = useLoaderData({ from: '/_authenticated/courses/$courseId/students/$studentId' })
    const { student, course, section } = data
    console.log("StudentDetail: ", student, course, section)
    return (
        
        <div className="container mx-auto">
            <p className="text-lg font-medium my-5 text-center">{student.first_name} {student.last_name}
