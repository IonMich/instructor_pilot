...
import { useDeleteAllSubmissionsMutation } from "@/utils/queryOptions"

import { columns } from "./columns"
import { DataTable } from "./submissions-data-table"

import { LuFile, LuFilePlus2, LuMoreHorizontal, LuFileBarChart2, LuFileScan, } from "react-icons/lu"
...
export function SubmissionsTable() {
    const data = useLoaderData({ from: '/_authenticated/assignments/$assignmentId' })
    const { assignment, submissions } = data
    return (
      <>
        <div className="container mx-auto py-2">
          <h1 className="text-5xl font-bold py-8">{assignment?.name}</h1>
          <AssignmentDetailCards assignment={assignment} submissions={submissions} />
...
function AssignmentDetailCards({
  ...
  submissions: Submission[]
    ...
    const subsGraded = submissions.filter(submission => submission.grade !== null) || []
    const subsIdentified = submissions.filter(submission => submission.student !== null) || []
    const subsAverage = subsGraded.map(submission => submission.grade).reduce((a, b) => a + b, 0) / subsGraded.length
    const percentGraded = assignment?.submission_count? parseInt((subsGraded.length / assignment?.submission_count * 100).toFixed(0)) : 0
    const percentIdentified = assignment?.submission_count? parseInt((subsIdentified.length / assignment?.submission_count * 100).toFixed(0)) : 0
    return (
          ...
          <CardContent className="flex flex-row justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {assignment?.max_question_scores.split(",").length} question{assignment?.max_question_scores.split(",").length? assignment?.max_question_scores.split(",").length > 1 ? "s" : "" : ""}{" "}
              </p>
            </div>
            <div className="mt-8">
...
function AllSubmissionsDropdownMenu({assignment}: {assignment: Assignment}) {
  ...
  return (
    <DialogContent className="sm:max-w-[425px]">
      ...
      <div className="grid gap-4 py-4">
        <DialogDescription>
          Are you sure you want to delete all {assignment.submission_count} submissions for assignment {assignment.name}?
        </DialogDescription>
      </div>
      <DialogFooter>
