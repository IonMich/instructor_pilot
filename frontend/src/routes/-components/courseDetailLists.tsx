...
export function SectionList({ sections }: { sections: Section[] }) {
    return <>
        {sections.map((section) => (
            <Link to="/sections/$sectionId" params={{ sectionId: section.id }} className="flex items-center first:mt-2 hover:bg-muted/50 p-4 rounded-lg duration-200" key={section.id}>
...
export function AssignmentList({ assignments }: { assignments: Assignment[] }) {
    ...
    const preferredKey = assignments.find((assignment) => assignment.assignment_group_object.name === userPreferedGroup)?.assignment_group_object.id
    const [tabValue, setTabValue] = React.useState(preferredKey)
    const assignmentGroups = assignments.reduce((acc, assignment) => {
